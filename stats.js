...
const main = async function() {
    ...
    (await root.once('value')).forEach(childSnapshot => {
        const task = (async function () {
            ...
            process.stdout.write(`${childSnapshot.key}\t${room.url?.value}\t${room.currentTime?.value}\t${room.paused?.value}\t${room.isLocalMode?.value}\t${room.minutesWatched?.value}\t${room.currentTime?.updatedAt && new Date(room.currentTime?.updatedAt * 1000).toISOString()}\t${room.createdAt && new Date(room.createdAt * 1000).toISOString()}\n`)
